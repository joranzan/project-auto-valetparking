...
class lc_path_pub :
    def __init__(self):
        ...
        rospy.Subscriber(object_topic_name, ObjectStatus, self.object_info_callback)
            ...
            for i in range(len(global_vaild_object)):
                for path in ref_path.poses :   
                    if global_vaild_object[i][0]==1 or global_vaild_object[i][0]==2 :  
                        dx = path.pose.position.x - global_vaild_object[i][1]
                        dy = path.pose.position.y - global_vaild_object[i][2]
                        dis = sqrt(dx**2 + dy**2)

                        if dis<3.5:
                            local_dx = local_vaild_object[i][1]
                            local_dy = local_vaild_object[i][2]
                            rel_distance = sqrt(local_dx**2 + local_dy**2)                     
