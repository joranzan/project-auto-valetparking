    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_path and self.is_status and self.is_obj:
                if self.checkObject(self.local_path, self.object_data) and self.lat_on == False:
                    
                    self.lattice_path = self.latticePlanner(self.local_path, self.status_msg)
                    lattice_path_index = self.collision_check(self.object_data, self.lattice_path)

                    #TODO: (7) lattice 경로 메세지 Publish
                    self.lattice_path_pub.publish(self.lattice_path[lattice_path_index])
                elif self.lat_on == True:
                    lattice_path_index = self.collision_check(self.object_data, self.lattice_path)
                    self.lattice_path_pub.publish(self.lattice_path[lattice_path_index])
                else:
    ...
    def latticePlanner(self,ref_path, vehicle_status):
        ...
        if len(ref_path.poses) > look_distance :
            ...
            for end_point in local_lattice_points :
                ...
                x_interval = 0.05 # 생성할 Path 의 Point 간격을 0.5 로 한다.
                x_start=0
                x_end=end_point[0]
                y_start = 0.0
                y_end = end_point[1]
