...
class ROSDemo:
    def __init__(self):
        ...
        pkg_path = rospack.get_path('rrt_drive1')
        ...
        rospy.Subscriber("/start_rrt", String, self.rrt_start_callback)
        ...
        while self.status_msg.velocity.x > 0.1:
            continue

        while not rospy.is_shutdown():
            if self.is_odom and self.is_status and self.car is None and self.is_obj and self.is_rrt_started:
    ...
    def rrt_start_callback(self, msg):
    ...
    def create_target(self):
        ...
        tgt.scale.x, tgt.scale.y, tgt.scale.z = 1.5, 1.5, 1.5
        tgt.pose.position.x, tgt.pose.position.y, tgt.pose.position.z = -10.110463829, 1039.80720215, self.z  # 전진 주차
        # tgt.pose.position.x, tgt.pose.position.y, tgt.pose.position.z = -8.40, 1038.44, self.z
