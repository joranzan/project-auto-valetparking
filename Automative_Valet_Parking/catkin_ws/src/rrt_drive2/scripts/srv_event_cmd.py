...
is_parking_started = False

def srv_client():
    rospy.init_node('MoraiEventCmd_client', anonymous=True)
    #(1) Service 가 생성 대기 함수 선언
    rospy.wait_for_service('/Service_MoraiEventCmd')
    rospy.Subscriber( '/mode_cmd' , EventInfo , mode_callback)
    rospy.Subscriber('/ready_to_park', String, ready_to_park_callback)
    global set_Event_control
    set_Event_control = EventInfo()

    global is_parking_started
...
def mode_callback(msg):
    # gear 값을 변경
    set_Event_control.gear = msg.gear

def ready_to_park_callback(msg):
    global is_parking_started
    if msg.data == "Completed":
        is_parking_started = True
