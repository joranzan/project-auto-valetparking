...
class pure_pursuit :
    ...
    def calc_pure_pursuit(self,):
        ...
        '''
        self.lfd = abs(self.lfd_gain * self.status_msg.velocity.x)  #적절한 lfd값 찾아야함
        ...
        for num,i in enumerate(self.path.poses) :
            ...
            # ex) [15.80591636 -1.48426122  1.        ]
            # 각도를 계산하여 후방에 있는지 확인
            # print('??',local_path_point)
            # print(local_path_point[1], local_path_point[0])


            if local_path_point[0] < 0:
                angle_to_point = atan2(local_path_point[1], local_path_point[0])
                if abs(angle_to_point) > (pi/2) and abs(angle_to_point) < pi:  # 정면에 있는 경우에만
                    #후방주시지점 찾기
                    #차량좌표계 기준 차량과 경로점 사이의 거리
                    dis = sqrt(local_path_point[0] ** 2 + local_path_point[1] ** 2)
                    # print('angle_to_point' , abs(angle_to_point))
                    # print(dis)
                    # print(self.lfd)
                    if dis >= self.lfd : # 전방주시거리보다 멀리 떨어진 점 찾기
                        # print('angle : ', angle_to_point)
                        # print('distance',dis)
                        # print('lfd',self.lfd)
                        # print('limit', 2*pi/3)
        ...
        # theta = atan2(self.forward_point[1], abs(self.forward_point[0]))
