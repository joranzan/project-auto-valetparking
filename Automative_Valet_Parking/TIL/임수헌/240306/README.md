# Sub PJT 2 실습
- [Sub PJT 2 실습](#sub-pjt-2-실습)
  - [1. gps\_parser.py](#1-gps_parserpy)
    - [이슈](#이슈)
    - [코드](#코드)
    - [실행결과](#실행결과)
  - [2. gpsimu\_parser.py](#2-gpsimu_parserpy)
    - [코드](#코드-1)
    - [실행결과](#실행결과-1)
  - [3. tf\_pub.py](#3-tf_pubpy)
    - [코드](#코드-2)
    - [실행결과](#실행결과-2)
  - [4. mgeo\_pub.py](#4-mgeo_pubpy)
    - [이슈](#이슈-1)
    - [코드](#코드-3)
    - [실행 결과](#실행-결과)
...
## 1. gps_parser.py
> 시뮬레이터에서 출력되는 GPS 센서 데이터를 받아 UTM으로 변환 (Pyproj 라이브러리 활용)
### 이슈
```python
xy_zone = self.proj_UTM(self.lat, self.lon) 가 아니라
xy_zone = self.proj_UTM(self.lon, self.lat) 해야함
```
...
class LL2UTMConverter:
    def __init__(self, zone=10) :
        ...
        self.proj_UTM = Proj(proj= , zone= , ellps =  , preserve_units =)
...
```
### 실행결과
![Alt text](image/gps_parser.PNG)

## 2. gpsimu_parser.py
- frame_id
  > 해당 메시지가 어떤 frame에 속해있는지를 의미함
- frame
  > 좌표계의 이름
  - base_link : 로봇(자동차) 자체의 위치를 나타내는 frame

https://velog.io/@7cmdehdrb/whatIsTF |
https://velog.io/@bbolddagu/ROS2-Odometry 참고
...
```
### 실행결과
![Alt text](image/gpsimu.PNG)

## 3. tf_pub.py
- TF : ROS에서 각각의 좌표계끼리의 관계를 정의

https://velog.io/@7cmdehdrb/whatIsTF 참고
...
$ rqt -> rqt 실행
```
![Alt text](image/tf_settings.PNG)
...
## 4. mgeo_pub.py
### 이슈
- rviz 실행 이슈 해결 방법 (Segmentation Fault)
  ```bash
    ...
    $ source ~/.bashrc
  ```
  https://github.com/ros-visualization/rviz/issues/1438 참고

- 데이터 확인
  - vscode에서 기존 debugger 지우고 Debugpy Old extensions 설치
  - vscode로 mgeo 디버그

![Alt text](image/link_debug.PNG) | ![Alt text](image/nodes_debuh.PNG)
...
class get_mgeo :
    ...
    def getAllLinks(self):
        ...
        '''
        for lines in self.links.values() :
